module.exports = function (grunt) {
  ...
  grunt.initConfig({
    ...
    uglify: {
      ...
      core: {
        files: {
          'tmp/dust-core.min.js': ['<%= concat.core.dest %>'],
        },
      },
      full: {
        files: {
          'tmp/dust-full.min.js': ['<%= concat.full.dest %>'],
        },
      },
    },
    clean: {
    ...
    log: {
      coverage: {
        options: {
          message:
            'Open <%=jasmine.coverage.options.templateOptions.report%>/index.html in a browser to view the coverage.',
        },
      },
      copyForRelease: {
        options: {
      ...
      release: {
        options: {
          message: [
            'OK. Done bumping, adding, committing, tagging and pushing the new version',
            '',
  ...
  grunt.registerTask('test', ['build', 'jasmine:testProd', 'testCli', 'testNode', 'jasmine:coverage']);

  //sauce labs integration (browser testing)
  grunt.registerTask(
    'sauce',
    process.env.SAUCE_ACCESS_KEY ? ['jasmine:testProd:build', 'connect:testServer', 'saucelabs-jasmine'] : []
  );
